...
return (
     <nav style={{fontFamily: 'Poppins, sans-serif'}} className="bg-cyan-50 px-2 sm:px-4 py-2.5 rounded  z-10 ">
      ...
      {isListOpen && (
        <ul className="dropdown-list" onMouseLeave={() => setIsListOpen(false)}>
        <li>
          <button type="button" className="hover:text-orange-300" onClick={() => Navigate("/about")}>About Us</button>
        </li>
        <li>
          <button type="button" className="hover:text-orange-300"  onClick={() => Navigate("/team")}>Our Team </button>
        </li>
        <li>
          <button type="button" className="hover:text-orange-300"  onClick={() => Navigate("/careers")}>Careers </button>
        </li>
      </ul>
      )}
    </div>
