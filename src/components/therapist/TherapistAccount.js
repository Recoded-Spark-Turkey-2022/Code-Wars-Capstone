...
function TherapistAccount() {
  ...
  const handleSubmit = (event) => {
    ...
    dispatch(
      createTherapistProfile({ Licensenumber, City, Username, Email, Password })
    );
    ...
    if (Password !== confirmPassword) {
      ...
      firebaseError.message === 'Firebase: Error (auth/invalid-email).'
    ) {
      ...
      console.table(Username, Email, City, Licensenumber, Password);
    }
  };
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="ml-20 w-6/12 text-slate-500">
        ...
        <br />
        {firebaseError && (
          <div style={{ color: 'red' }}>{firebaseError.message}</div>
        )}
        {error && <div style={{ color: 'red' }}>{error}</div>}
        <button
          type="submit"
          onSubmit={handleSubmit}
          className="lg:text-2xl md:text-1xl sm:text-sm rounded-md box-border py-2 lg:px-10 md:px-4 sm:px-2 transition-all duration-250 bg-cyan-400 hover:bg-cyan-500 hover:text-white text-black mt-10"
        >
          Create
        </button>
      </form>
